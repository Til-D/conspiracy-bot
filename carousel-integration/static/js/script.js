...
function restartConversation() {
    console.log('-restartConversation');
    $("#userInput").prop('disabled', true);
...
function action_trigger() {
    console.log('action_trigger: ' + action_name);
...
function showCardsCarousel(cardsToAdd) {
    ...
    var all_cards = document.querySelectorAll(".paginated_cards");
    var all_card_scroller = document.querySelectorAll(".cards_scroller");
    var card_item_size = 225;

    var arrowsNext = document.querySelectorAll(".arrow.next")
    var arrowsPrev = document.querySelectorAll(".arrow.prev");

    // remove all listeners and add only to the last class rendered
    for(var i=0; i<arrowsNext.length; i++) {
        if(i==arrowsNext.length-1) {
            // console.log('+ adding click listener');
            arrowsNext[i].addEventListener("click", scrollToNextPage);
            arrowsPrev[i].addEventListener("click", scrollToPrevPage);    
            // console.log(arrowsNext[i]);
            // console.log(arrowsPrev[i]);
            card = all_cards[i];
            card_scroller = all_card_scroller[i];
        } else {
            // console.log('+ removing click listener');
            arrowsNext[i].removeEventListener("click", scrollToNextPage);
            arrowsPrev[i].removeEventListener("click", scrollToPrevPage);    
            // console.log(arrowsNext[i]);
            // console.log(arrowsPrev[i]);
        }
    }

    // console.log(arrowsNext);
...
function createCardsCarousel(cardsData) {
    ...
    var cardContents = '<div class="cards paginated_cards"> <div class="cards_scroller">' + cards + '  <span class="arrow prev fa fa-chevron-circle-left "></span> <span class="arrow next fa fa-chevron-circle-right" ></span> </div> </div>';
...
$(document).on("click", ".article", function() {
    ...
    // setUserResponse(text);
    // action_name = payload;
    send('/article_summary{"article_id": "' + articleId + '"}');
    // //delete the quickreplies
    // $(".quickReplies").remove();
